...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        args = list(args)
        args = split_tensors(group_size, args)
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(self.seq_length, bdash, self.vocab_size + 1).zero_() for _ in
                                   range(group_size)]
                ...
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if remove_bad_endings and t - divm > 0:
                        logprobsf[torch.from_numpy(
                            np.isin(beam_seq_table[divm][t - divm - 1].cpu().numpy(), self.bad_endings_ix)), 0] = float(
                            '-inf')
